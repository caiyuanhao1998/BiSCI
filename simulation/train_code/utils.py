...
def LoadTraining(path):
    imgs = []
    scene_list = os.listdir(path)  #返回指定路径下的文件和文件夹列表
    scene_list.sort()
    ...
    # for i in range(5):
        ...
        if scene_num<=205:
            if 'mat' not in scene_path:
                continue  #作用是不执行循环体剩余部分，直接进行下次循环
            img_dict = sio.loadmat(scene_path)
            if "img_expand" in img_dict:
                img = img_dict['img_expand'] / 65536.  #256^2  0-255 matlab中的归一化
            elif "img" in img_dict:
                img = img_dict['img'] / 65536.
            img = img.astype(np.float32)  #图片转化为float2类型
            imgs.append(img)
...
def checkpoint(model, epoch, model_path, logger):
    ...
    torch.save(model.state_dict(), model_out_path)   #保存模型参数字典
