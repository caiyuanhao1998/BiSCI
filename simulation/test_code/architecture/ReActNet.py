...
class BinaryConv2d_Down(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1,groups=1,bias=False):
        ...
        if inplanes == planes:
            self.binary_pw = binaryconv3x3(inplanes, planes,groups=groups,bias=bias)
...
class BiUNet_ReActNet_body(nn.Module):
    def __init__(self, in_dim=28, out_dim=28, dim=28, stage=2, num_blocks=[2,4,4]):
        super(BiUNet_ReActNet_body, self).__init__()
        self.dim = dim
        self.stage = stage
        ...
        for i in range(stage):
            self.encoder_layers.append(nn.ModuleList([
                ReActNet_block(dim=dim_stage, num_blocks=num_blocks[i], dim_head=dim, heads=dim_stage // dim),
                BinaryConv2d_Down(dim_stage, dim_stage * 2, 1, bias=False),
            ]))
            dim_stage *= 2

        # Bottleneck
        self.bottleneck = ReActNet_block(
    ...
    def forward(self, x):
        ...
        # Encoder
        fea_encoder = []
        for (ReActNet_block, FeaDownSample) in self.encoder_layers:
            # stx()
            fea = ReActNet_block(fea)
        ...
        # Decoder
        for i, (FeaUpSample, Fution, ReActNet_block) in enumerate(self.decoder_layers):
            fea = FeaUpSample(fea)
...
class ReActNet(nn.Module):
    ...
    def __init__(self, in_channels=28, out_channels=28, n_feat=28, stage=3, num_blocks=[1,1,1]):
        ...
        modules_body = [BiUNet_ReActNet_body(dim=n_feat, stage=2, num_blocks=num_blocks) for _ in range(stage)]
...
#     model = BiUNet_ReActNet_3L_v2(stage=1,num_blocks=[1,1,1]).cuda()
