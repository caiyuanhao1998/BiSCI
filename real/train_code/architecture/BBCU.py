...
class BiUNet_BBCU_body(nn.Module):
    def __init__(self, in_dim=28, out_dim=28, dim=28, stage=2, num_blocks=[2,4,4]):
        ...
        for i in range(stage):
            self.encoder_layers.append(nn.ModuleList([
                BBCU_Block(dim=dim_stage, num_blocks=num_blocks[i], dim_head=dim, heads=dim_stage // dim),
                BinaryConv2d_Down(dim_stage, dim_stage * 2, 1, bias=False),
            ]))
            dim_stage *= 2

        # Bottleneck
        self.bottleneck = BBCU_Block(
    ...
    def forward(self, x):
        ...
        # Encoder
        fea_encoder = []
        for (BBCU_Block, FeaDownSample) in self.encoder_layers:
            # stx()
            fea = BBCU_Block(fea)
        ...
        # Decoder
        for i, (FeaUpSample, Fution, BBCU_Block) in enumerate(self.decoder_layers):
            fea = FeaUpSample(fea)
...
class BBCU(nn.Module):
    '''
    Only 3 layers are 32-bit conv
    '''
    def __init__(self, in_channels=28, out_channels=28, n_feat=28, stage=3, num_blocks=[1,1,1],k=14):
        super(BBCU, self).__init__()
        self.stage = stage
        self.conv_in = nn.Conv2d(in_channels, n_feat, kernel_size=3, padding=(3 - 1) // 2,bias=False)       # 1-bit -> 32-bit
        modules_body = [BiUNet_BBCU_body(dim=n_feat, stage=2, num_blocks=num_blocks) for _ in range(stage)]
...
#     model = BiUNet_BBCU_3L_v2(stage=1,num_blocks=[1,1,1]).cuda()
